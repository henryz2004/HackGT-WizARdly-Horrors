...
export class EnemyBatBehaviour extends BaseScriptComponent {
    ...
    private move(): void {
        ...
        const newPos: vec3 = currPos.add(
            dir_vec.uniformScale(this.forwardSpeed * getDeltaTime()).add(perpendicular.uniformScale(this.strafeSpeed * getDeltaTime())) )

        this.enemy.getTransform().setWorldPosition(newPos)

        this.enemy.getTransform().setWorldRotation(quat.lookAt(dir_vec, vec3.up()))
    }
}
